...
const fromBody = {
  id: "p1",
  elements: { orbitRadiusAu: 1, eccentricity: 0, periapsisAngle: 0, orbitalPhase: 0 },
  endpoint: { mu: 3.986e14, radiusM: 6.371e6 },
};
const toBody = {
  id: "p2",
  elements: { orbitRadiusAu: 1.524, eccentricity: 0, periapsisAngle: 0, orbitalPhase: 0.5 },
  endpoint: { mu: 4.28e13, radiusM: 3.39e6 },
};

Deno.test("findDirectRoutes: returns a self-consistent orbit→orbit route", () => {
  const routes = findDirectRoutes(fromBody, toBody, EndState.Orbit, EndState.Orbit, MU, "star", {
  ...
  const sumTerminals = r.nodes[0].terminal!.totalDeltaV + r.nodes[1].terminal!.totalDeltaV;
...
Deno.test("findDirectRoutes: intercept destination is cheaper than orbit", () => {
  const orbit = findDirectRoutes(fromBody, toBody, EndState.Orbit, EndState.Orbit, MU, "star", {})[0];
  const intercept = findDirectRoutes(fromBody, toBody, EndState.Orbit, EndState.Intercept, MU, "star", {})[0];
...
Deno.test("findDirectRoutes: pareto set is Δv-sorted and non-dominated", () => {
  ...
  for (let i = 1; i < routes.length; i++) {
    if (routes[i].totalDeltaV < routes[i - 1].totalDeltaV) throw new Error("not Δv-sorted");
    if (routes[i].duration >= routes[i - 1].duration) throw new Error("not non-dominated");
  }
});
